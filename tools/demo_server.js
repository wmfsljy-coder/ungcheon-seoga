...
window.DemoServer=(function(){
  ...
  function seed(){
    var wk=Core.weekKey(new Date(),0),pw=Core.weekKey(new Date(),-1),T={};
    ...
    T["공감"].push({"시각":wk,"글id":"g1","누른이":env.hmac("like|2101|g1")});
    function post(id,week,kind,hb,name,cls,title,author,text,head,printed){
      T["글"].push({"id":id,"시각":week+" 10:00:00","주":week,"차수":"1","종류":kind,"학번":hb,"이름":name,"반":cls,"도서id":"free",
        "책제목":title,"지은이":author,"머리글":head||"","본문":text,"계기":"추석 연휴에 도서관에서 빌려 읽음","쪽수":kind==="review"?"132쪽":"",
    ...
    [["g2","3101"],["g2","3102"],["g2","3301"],["g3","3501"],["g3","3202"],["g1","3502"]].forEach(function(v){
      T["투표"].push({"시각":wk,"주":wk,"종류":"label","투표자":env.hmac("vote|"+v[1]+"|"+wk+"|label"),"글id":v[0]});});
    /* 김서준: 이번 주 도장 5개 + 책갈피 1장, 지난달 상품권 대상 */
    var lm=Core.prevMonth(wk.slice(0,7));
    [["코스모스","칼 세이건"],["데미안","헤르만 헤세"],["하얼빈","김훈"],["1984","조지 오웰"],["파친코","이민진"],["연금술사","파울로 코엘료"]].forEach(function(b,i){
      post("d"+i,wk,i===2?"review":"label","3201","김서준","3-2",b[0],b[1],"읽기 시작하면 멈출 수 없다. 이번 주에 읽은 책 가운데 가장 오래 남았다. 추천한다!",i===2?"오래 남는 책":"");
      T["글"][T["글"].length-1]["시각"]=wk+" 0"+(i+1)+":00:00";});
