...
EMAIL="lib@x";C().api("eventSet",{name:"가을 독서 주간",from:"2026-10-06",to:"2026-10-09",mul:2,target:"1학년",memo:"1학년 도장 두 배"});
...
must(a.event&&a.event.mul===2&&a.event.name==="가을 독서 주간","학생 화면에 진행 중인 이벤트가 보인다");
/* 대상이 아닌 학년은 그대로 */
post("2101","label","2026-10-06");
must(st("2101").week.stamps.length===1,"2학년은 대상이 아니라 그대로 1개");
must(!st("2101").event,"대상이 아니면 이벤트 알림도 없다");
...
/* 종류를 정하면 그 종류만 두 배 */
EMAIL="lib@x";C().api("eventSet",{name:"독후감 주간",from:"2026-10-12",to:"2026-10-16",mul:2,kind:"review"});
...
must(!st("2101").event,"내리면 바로 꺼진다");
/* 잘못된 값 */
const err=f=>{try{f();return "";}catch(e){return e.message;}};
EMAIL="lib@x";
must(/배수는 2에서 5/.test(err(()=>C().api("eventSet",{name:"x",from:"2026-11-01",to:"2026-11-02",mul:9}))),"배수는 2~5");
must(/끝 날짜/.test(err(()=>C().api("eventSet",{name:"x",from:"2026-11-05",to:"2026-11-01",mul:2}))),"끝이 시작보다 빠르면 거부");
EMAIL="s1101@x";
must(/관리자/.test(err(()=>C().api("eventSet",{name:"x",from:"2026-11-01",to:"2026-11-02",mul:2}))),"학생은 이벤트를 못 연다");
